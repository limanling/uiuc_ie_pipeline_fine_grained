...
docker run --rm -v ${data_root}:${data_root} -i --network="host" --gpus device=1 limanling/uiuc_ie_m36 \
...
docker run --rm -v ${data_root}:${data_root}  -i --gpus '"device=1"' limanling/uiuc_ie_m36 \
    /opt/conda/envs/py36/bin/python \
