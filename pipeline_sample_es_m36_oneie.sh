...
# edl_cfet_json=${data_root}/mention/spanish.nam.cfet.json
edl_tab_nam_bio=${data_root}/mention/spanish.nam.bio
...
docker run --rm -v ${data_root}:${data_root} -w `pwd` -i --network="host" --gpus device=2 limanling/uiuc_ie_m36 \
...
docker run --rm -v ${data_root}:${data_root} -w `pwd` -i --gpus device=2 limanling/uiuc_ie_m36 \
    /opt/conda/envs/py36/bin/python \
