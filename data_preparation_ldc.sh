...
# unzip ltf
docker run --rm -v ${ldc_data}:${ldc_data} -v ${ltf_dir}:${ltf_dir} -i limanling/uiuc_ie_m18 \
    mkdir -p ${ltf_dir}
docker run --rm -v ${ldc_data}:${ldc_data} -v ${ltf_dir}:${ltf_dir} -i limanling/uiuc_ie_m18 \
    unzip -o ${ldc_data}/data/ltf/\*.zip -d ${ltf_dir}
# generate rsd
docker run --rm -v ${ldc_data}:${ldc_data} -v ${ltf_dir}:${ltf_dir} -v ${rsd_dir}:${rsd_dir} -w `pwd` -i limanling/uiuc_ie_m18 \
