...
docker run --rm -v `pwd`:`pwd` -w `pwd` -i limanling/uiuc_ie_m18 \
    ...
    ./system/aida_utilities/ltf2bio.py ${ltf_source} ${edl_bio}
# generate file list
docker run --rm -v `pwd`:`pwd` -w `pwd` -i limanling/uiuc_ie_m18 \
    /opt/conda/envs/py36/bin/python \
    ./system/aida_utilities/dir_readlink.py ${rsd_source} ${rsd_file_list}
docker run --rm -v `pwd`:`pwd` -w `pwd` -i limanling/uiuc_ie_m18 \
    /opt/conda/envs/py36/bin/python \
    ./system/aida_utilities/dir_ls.py ${ltf_source} ${ltf_file_list}

# apply stanford corenlp
docker run --rm -v `pwd`:`pwd` -w `pwd` -i limanling/uiuc_ie_m18 \
    /opt/conda/envs/py36/bin/python \
    ./system/aida_utilities/parent_child_util.py \
