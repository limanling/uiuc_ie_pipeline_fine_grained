...
event_coarse_without_time=${event_result_dir}/en.event.cs
...
event_filter=${event_result_dir}/events_nohypothetical.cs
...
event_final=${event_result_dir}/events_info.cs  #dryrun_event_with_times.cs 
...
#     --eval m36
# # echo "statistics before fine"
# # docker run --rm -v ${data_root}:${data_root} -w `pwd` -i --network="host" limanling/uiuc_ie_m36 \
# #     /opt/conda/envs/py36/bin/python \
# #     /postprocessing/statistics_cs.py \
# #     ${edl_cs_coarse}
# # echo "statistics after fine"
# # docker run --rm -v ${data_root}:${data_root} -w `pwd` -i --network="host" limanling/uiuc_ie_m36 \
# #     /opt/conda/envs/py36/bin/python \
# #     /postprocessing/statistics_cs.py \
# #     ${edl_cs_fine}
...
docker run --rm -v ${data_root}:${data_root} laituan245/aida_attrs_filter \
    -i ${event_fine_all_clean} -s lifu -o ${event_filter} -l ${ltf_source}
# Event coreference
echo "** Event coreference **"
docker run --rm -v ${data_root}:${data_root} -w `pwd` -i --network="host" limanling/uiuc_ie_m36 \
    /opt/conda/envs/py36/bin/python \
    /event/aida_event_coreference/gail_event_coreference_test.py \
    -i ${event_filter} -o ${event_corefer} -c ${event_corefer_confidence} -r ${rsd_source} -l ${lang}
# ###### add haoyang's
